		...
		let mysqlPromise = function (email) {
			return new Promise((resolve, reject) => {
				mysql_connection.query(
					`SELECT * FROM user WHERE email= ${mysql.escape(email)} and verified = true LIMIT 1`,
...
custom_validators.isUsernameAvailable = function () {
	return function (value) {
		let mysqlPromise = function (username) {
			return new Promise((resolve, reject) => {
				mysql_connection.query(
					`SELECT * FROM user WHERE username=${mysql.escape(username)} and verified = true LIMIT 1`,
					function (error, results) {
						if (error !== null) throw new Error(error);
