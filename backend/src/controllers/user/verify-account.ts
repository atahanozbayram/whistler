...
const verifAccCtrllr = function (req: TypedRequestBody<verifAccReqBody>, res: Response) {
			...
			prisma.user_verification
				...
				.then((verification) => {
					...
					prisma.user.update({ where: { uuid: user1.uuid }, data: { verified: true } }).then(() => {
						...
						prisma.user_verification.update({ where: { uuid: verification.uuid }, data: { valid: false } }).then(() => {
...
verifAccRoute.use(reqLgrMwCreator(), verifAccValChain, validationCheckMw, verifAccCtrllr);
export { verifAccRoute, verifAccReqBody };
